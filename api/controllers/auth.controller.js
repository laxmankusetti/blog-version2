...
export const signup = async (req, res, next) => {
  try {
    ...
    res.status(201).json("User signin successfull");
  } catch (error) {
    next(error);
  }
};
...
export const signin = async (req, res, next) => {
  ...
  try {
    ...
    if (!validPassword) {
      return next(400, "Invalid credientials");
    }

    const { password: pass, ...rest } = validUser._doc;

    const token = jwt.sign({ id: validUser._id, isAdmin: validUser.idAdmin }, process.env.JWT_SECRET_KEY);
    res
      .status(200)
